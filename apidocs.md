...
Assignment {
  ...
  startTime (Number|undefined, required): The start time of the assignment
...
NewAssignment {
  ...
  startTime (Number|undefined, optional): The start time of the assignment
...
For example, if the markingCriteriaID that is given doesn't exist but is sumbitted, the intention would be to update the none existant criteria.  This is an errpneous request.
##### Response schema
```
Error {
  message (String, optional): Explanation of the error
